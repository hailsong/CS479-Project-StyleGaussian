# USAGE !!
'''
python render_style_camera_original.py -m output/saengsaeng/artistic/default --style_img_path images/22.jpg --target_style_img_path images/43.jpg --steps 121 --interp_per_cam 5
python render_style_camera_original.py   -m output/saengsaeng/artistic/default   --style_img_path images/22.jpg   --target_style_img_path images/43.jpg   --steps 121
'''
'''
...
def save_override_color_as_ply(gaussians: GaussianModel, override_color: torch.Tensor, path: str):
    """Save override_color as SH-DC in PLY using a dummy SH structure."""
    if override_color.dim() == 2 and override_color.shape[1] == 3:
        override_color = override_color.unsqueeze(-1)  # (P, 3) → (P, 3, 1)

    elif override_color.dim() == 4:
        override_color = override_color.squeeze(0).permute(1, 0, 2).contiguous()  # (P, 3, 1)

    elif override_color.dim() == 3:
        if override_color.shape[0] == 1:
            override_color = override_color.squeeze(0).permute(1, 0, 2).contiguous()
        elif override_color.shape[2] == 1:
            override_color = override_color.permute(1, 0, 2).contiguous()
        else:
            raise ValueError(f"3D override_color shape not handled: {override_color.shape}")

    else:
        raise ValueError(f"Unexpected override_color shape: {override_color.shape}")

    assert override_color.shape[1] == 3 and override_color.shape[2] == 1, "Expected shape (P, 3, 1)"

    P = override_color.shape[0]
    D = (gaussians.max_sh_degree + 1) ** 2 - 1
    device = override_color.device

    gaussians._features_dc = override_color.transpose(1, 2).contiguous()  # (P, 1, 3)
    gaussians._features_rest = torch.zeros((P, 3, D), device=device)
...
def render_sets_style_camera_interpolate(dataset: ModelParams, pipeline: PipelineParams, style_img_path, steps=10, interp_per_cam=5, target_style_img_path=None):
    with torch.no_grad():
        ...
        for i in tqdm(range(total_frames), desc="Rendering camera+style interpolation"):
            ...
            # # Save Gaussian PLY at first frame
            # if i == 0:
            #     save_override_color_as_ply(gaussians, override_color, os.path.join(dataset.model_path, f"gaussian_{style_index}.ply"))

            # # Save Gaussian PLY at last frame
            # if i == total_frames - 1:
            #     save_override_color_as_ply(gaussians, override_color, os.path.join(dataset.model_path, f"gaussian_{target_index}.ply"))
